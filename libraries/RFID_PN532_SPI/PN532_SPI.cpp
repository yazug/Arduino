...
/*
 *
 * modified to work w/ SPI library by Sin
 *
 */
 ...
 */
/**************************************************************************/
Adafruit_PN532_SPI::Adafruit_PN532_SPI(const uint8_t ss) {
	_ss = ss;
}
 ...
 */
/**************************************************************************/
void Adafruit_PN532_SPI::PrintHex(const byte * data, const uint32_t numBytes) {
	uint32_t szPos;
 ...
 */
/**************************************************************************/
void Adafruit_PN532_SPI::PrintHexChar(const byte * data, const uint32_t numBytes) {
	uint32_t szPos;
	for (szPos = 0; szPos < numBytes; szPos++) {
		// Append leading 0 for small values
		if (data[szPos] <= 0xF)
 ...
 */
/**************************************************************************/
uint32_t Adafruit_PN532_SPI::getFirmwareVersion(void) {
	uint32_t response;
...
boolean Adafruit_PN532_SPI::sendCommandCheckAck(uint8_t *cmd, uint8_t cmdlen,
		uint16_t timeout) {
	uint16_t timer = 0;
	...
	while (/* readspistatus() */ status() != PN532_SPI_READY) {
		if (timeout != 0) {
			timer += 10;
			if (timer > timeout)
				return false;
 ...
 */
/**************************************************************************/
boolean Adafruit_PN532_SPI::writeGPIO(uint8_t pinstate) {
	uint8_t errorbit;
 ...
 */
/**************************************************************************/
uint8_t Adafruit_PN532_SPI::readGPIO(void) {
 ...
 */
/**************************************************************************/
boolean Adafruit_PN532_SPI::SAMConfig(void) {
 ...
 */
/**************************************************************************/
boolean Adafruit_PN532_SPI::readPassiveTargetID(uint8_t cardbaudrate, uint8_t * uid,
		uint8_t * uidLength) {
 ...
 */
/**************************************************************************/
bool Adafruit_PN532_SPI::mifareclassic_IsFirstBlock(uint32_t uiBlock) {
 ...
 */
/**************************************************************************/
bool Adafruit_PN532_SPI::mifareclassic_IsTrailerBlock(uint32_t uiBlock) {
	// Test if we are in the small or big sectors
	if (uiBlock < 128)
 ...
 */
/**************************************************************************/
uint8_t Adafruit_PN532_SPI::mifareclassic_AuthenticateBlock(uint8_t * uid,
	...
	if (pn532_packetbuffer[7] != 0x00) {
		...
		Adafruit_PN532_SPI::PrintHexChar(pn532_packetbuffer, 12);
#endif
		return 0;
	}
 ...
 */
/**************************************************************************/
uint8_t Adafruit_PN532_SPI::mifareclassic_ReadDataBlock(uint8_t blockNumber,
		uint8_t * data) {
#ifdef MIFAREDEBUG
	...
	if (pn532_packetbuffer[7] != 0x00) {
		...
		Adafruit_PN532_SPI::PrintHexChar(pn532_packetbuffer, 26);
		//#endif
		return 0;
	}
 ...
 */
/**************************************************************************/
uint8_t Adafruit_PN532_SPI::mifareclassic_WriteDataBlock(uint8_t blockNumber,
		uint8_t * data) {
#ifdef MIFAREDEBUG
 ...
 */
/**************************************************************************/
uint8_t Adafruit_PN532_SPI::mifareclassic_FormatNDEF(void) {
 ...
 */
/**************************************************************************/
uint8_t Adafruit_PN532_SPI::mifareclassic_WriteNDEFURI(uint8_t sectorNumber,
	...
	uint8_t sectorbuffer1[16] = { 0x00, 0x00, 0x03, len + 5, 0xD1, 0x01, len
 ...
 */
/**************************************************************************/
uint8_t Adafruit_PN532_SPI::mifareultralight_ReadPage(uint8_t page, uint8_t * buffer) {
	if (page >= 64) {
#ifdef MIFAREDEBUG
 ...
 */
/**************************************************************************/
boolean Adafruit_PN532_SPI::spi_readack() {
	uint8_t ackbuff[6];
...
/**************************************************************************/
/*
uint8_t Adafruit_PN532_SPI::readspistatus(void) {
	select();
//	delay(2);
	spiwrite(Adafruit_PN532_SPI_STATREAD);
	// read byte
	uint8_t x = spiread();
...
*/
inline uint8_t Adafruit_PN532_SPI::status() {
	uint8_t x;

	select();
	x = SPI.transfer(PN532_SPI_STATREAD);
	deselect();
	return x;
}
 ...
 */
/**************************************************************************/
void Adafruit_PN532_SPI::readspidata(uint8_t* buff, uint8_t n) {
	select();
//	delay(2);
 ...
 */
/**************************************************************************/
void Adafruit_PN532_SPI::spiwritecommand(uint8_t* cmd, uint8_t cmdlen) {
	uint8_t checksum;
...
void Adafruit_PN532_SPI::setupMode() {
	SPI.setClockDivider(SPI_CLOCK_DIV16); // max 5MHz
	//PN532 User Manual (UM0701-02) p. 25 of 200
	SPI.setBitOrder(LSBFIRST); // Data order is LSB first.
	SPI.setDataMode(SPI_MODE0); // SCK is active high (CPOL = 0), data is sampled on the first edge.
}

void Adafruit_PN532_SPI::select() {
	digitalWrite(_ss, LOW);
	//delay(2); // wake-up from sleep
}

void Adafruit_PN532_SPI::deselect() {
	digitalWrite(_ss, HIGH);
}

/*
inline void Adafruit_PN532_SPI::spiwrite(const uint8_t & c) {
	SPI.transfer(c);
}
*/
/**************************************************************************/
...
/**************************************************************************/
/*
inline uint8_t Adafruit_PN532_SPI::spiread(const uint8_t & c) {
